...
function initThemeToggle() {
  ...
  function updateColors(theme) {
    ...
    headerText.style.color = theme === 'light' ? '#000' : '#fff';
    header.style.backgroundColor = theme === 'light' ? 'rgb(0 0 0 / 5%)' : 'rgba(255, 255, 255, 0.05)';
    
    // Loop through all nav links and update their color
    navlinks.forEach(nav => {
      nav.style.color = theme === 'light' ? '#000' : '#fff';
    });
  }

  updateColors(savedTheme); // Apply the saved theme on page load
